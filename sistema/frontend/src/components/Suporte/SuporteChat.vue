<template>
    <div>
        ...
        <div v-if="abrirModal" class="position-fixed" style="bottom: 90px; right: 24px; z-index: 10000;">
            <div class="bg-white rounded-4 shadow d-flex flex-column" style="width: 400px; height: 500px; overflow: hidden;">
                <!-- Header -->
                <div class="d-flex align-items-center justify-content-between px-3 py-2" style="background-color: #0dc9ee;">
                    ...
                    <button class="btn btn-sm btn-close-white" @click="abrirModal = false">
                ...
                <div ref="chatBody" class="flex-grow-1 px-3 py-2 overflow-auto" style="background: #f9f9f9;">
                    <div class="text-muted small text-center my-2">
                        Pergunte qualquer coisa sobre o FlowTask.
                    </div>
...
const enviar = async () => {
    ...
    try {
        const { data } = await axios.post('/api/suporte/responder', { mensagem: pergunta })
        mensagens.value.push({ texto: formatarNegrito(data.resposta || 'Desculpe, não consegui responder agora.'), tipo: 'ia' })
    } catch {
